...
const jwtGen = (id, username, role) => {
  return jwt.sign({ id, username, role }, secret_key_jwt, { expiresIn: "60d" });
};
...
app.post("/login", async (req, res) => {
  if (!req.body)
    return res.send("Надо через боди данные отправлять, чел ! ! !");
  const { username, password } = req.body;
...
app.post("/addPost", verifyToken, async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Пост успешно создан.", post: newPost });
  } catch (err) {
    console.error(err);
